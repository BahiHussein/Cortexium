...
class NatsTransport {
    ...
    async subscribe(topic, handler, options = {}) {
        ...
        // Flush so the subscription interest has reached the server before we
        // resolve. This makes `await node.sub(...)` mean "ready to receive",
        // avoiding a race where an immediate request() sees no responders.
    ...
    async unsubscribe(topic, options = {}) {
        ...
        if (sub) {
            ...
            // Flush so interest removal has reached the server before we resolve.
            await this.connection.flush();
    ...
    /**
     * Send a streaming RPC request and yield each chunk as it arrives.
     *
     * Unlike request() (NATS's one-shot request/reply), this uses an explicit
     * inbox subscription so the responder can send MANY messages back. It is a
     * separate code path and does not touch the normal request() hot path.
     *
     * @param {string} topic
     * @param {Message} message
     * @param {Object} options
     * @param {number} options.idleTimeout - Max ms to wait between chunks (default 30000)
     * @returns {AsyncGenerator<*>} yields each chunk's payload until stream end
     */
    async *requestStream(topic, message, options = {}) {
        if (!this.connection) throw new Error('Transport not connected');

        const subject = this._subject(topic);
        const idleTimeout = options.idleTimeout || 30000;
        const inbox = createInbox();
        const sub = this.connection.subscribe(inbox);
        const { data, headers } = this._encode(message);

        // Fire the request with an explicit reply inbox (queue group still
        // load-balances to a single worker). The worker's msg.respond() calls
        // land on this long-lived subscription instead of a one-shot reply.
        this.connection.publish(subject, data, { reply: inbox, headers });
        debug.transport(`Stream request to ${subject} (inbox=${inbox}, idleTimeout=${idleTimeout}ms)`);
        ...
        try {
            for (;;) {
                ...
                const idle = new Promise((_, reject) => {
                    timer = setTimeout(() => {
                        reject(Object.assign(new Error(`Stream idle timeout after ${idleTimeout}ms`), { code: 'TIMEOUT' }));
                    }, idleTimeout);
                });

                let res;
                try {
                    res = await Promise.race([iterator.next(), idle]);
                } finally {
                    clearTimeout(timer);
                }
                ...
                // No responders: NATS posts a 503 status message to the reply inbox.
                const code = raw.headers && (raw.headers.code || (raw.headers.get && raw.headers.get('Status')));
                if (raw.data.length === 0 && (code === 503 || code === '503')) {
                    throw Object.assign(new Error(`No responders available for "${topic}"`), { code: 'NO_RESPONDERS' });
                }

                const replyMsg = this._decode(raw.data, raw);

                if (replyMsg.error) {
                    throw Object.assign(new Error(replyMsg.error.message), {
                        code: replyMsg.error.code,
                        details: replyMsg.error.details,
                    });
                }
                if (replyMsg.streamEnd) break;          // explicit end marker
                yield replyMsg.payload;
                if (!replyMsg.streamChunk) break;       // a plain reply => single value, done
            }
        } finally {
            // Runs on normal completion, thrown error, or consumer break/return.
